# folderbe : 유튜브 구독 채널을 폴더처럼

이 프로젝트는 이유는 제 **필요**에서 시작했습니다. 전 유튜브를 영어, 일본어, 프로그래밍 등 자기개발 용도로만 활용하고 싶었습니다. 하지만 유튜브는 맞춤 추천 영상으로 절 현혹합니다.  
...
유사 서비스로 크롬 확장인 pockettube가 있었다. 하지만 이 확장은  
1. 쓰기가 불편함
2. 어쨌든 유튜브 사이트를 들어가야함

이런 한계로 pockettube의 화면은 참고하되, 직접 웹 서비스로 만들기로 결정했다.
...
- [ ] 개발 or 배포 환경 config 분리
  - [ ] 인프라 개선
- [ ] 로그인 개선 - refresh token 기능 붙여서
  - + JWT 고려하기

로그인 부분의 이슈만 해결한 이후 실제 배포 진행하기

- [ ] 로깅 쉽게 연결
- [ ] 빌드, 배포 자동화
  - [ ] 인증서 자동 갱신
- [ ] 전체 채널 동기화 기능을 효율적으로 변경. 현재는 access_token이 refresh 되는 때마다인 1시간 간격으로 채널이 업데이트. 
